      ...
      <div className="pt-8 px-10 pb-20 max-w-6xl mx-auto">
        ...
        <header className="flex justify-between items-end mb-10">
          ...
          <button
            ...
            className="px-8 py-3 bg-primary-gradient rounded-full text-sm font-bold text-white shadow-[0_8px_30px_rgba(86,56,149,0.3)] flex items-center gap-2 hover:brightness-110 active:scale-95 transition-all cursor-pointer z-10"
          >
        ...
        <div className="flex justify-between items-center mb-8 pb-4 border-b border-white/5">
          <div className="flex bg-plasma-slate rounded-lg p-1 gap-1 border border-white/5">
            <button
              ...
              className={`px-6 py-2 rounded-lg text-xs font-bold tracking-widest uppercase transition-all cursor-pointer ${view === "calendar" ? "bg-plasma-primary text-white shadow-lg" : "text-plasma-text-secondary hover:text-plasma-text-primary"
                }`}
            >
              Calendar
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-lg text-xs font-bold tracking-widest uppercase transition-all cursor-pointer ${view === "list" ? "bg-plasma-primary text-white shadow-lg" : "text-plasma-text-secondary hover:text-plasma-text-primary"
                }`}
            >
              List
            </button>
              ...
              <div className="absolute right-0 top-12 bg-plasma-slate border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden min-w-[160px]">
                {intentOptions.map((opt) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm font-medium transition-colors cursor-pointer ${intentFilter === opt ? "bg-plasma-primary/15 text-plasma-primary" : "text-plasma-text-secondary hover:bg-white/5 hover:text-white"
                      }`}
                  >
                    {opt}
                  </button>
        ...
        {view === "calendar" ? (
                ...
                {Array.from({ length: 42 }).map((_, i) => {
                  ...
                  return (
                    <div
                      ...
                      className={`h-20 border-r border-b border-white/5 p-3 relative flex flex-col items-center justify-center cursor-pointer transition-all ${!isCurrentMonth ? "bg-plasma-bg/50 text-plasma-text-secondary/30" : "bg-plasma-slate hover:bg-plasma-slate-hover"
                    ...
                    >
                      <span className={`absolute top-3 left-3 text-sm ${isSelected ? "font-bold text-plasma-secondary" : isToday ? "font-bold text-plasma-primary" : "text-plasma-text-primary"
                        }`}>
                        {isCurrentMonth ? day : ""}
                      </span>
            ...
            <div className="flex-1 w-full space-y-6">
              ...
              {!loading && selectedDayEvents.length > 0 ? (
                selectedDayEvents.map(event => (
                  <div
                    key={event.id}
                    onClick={() => router.push(`/rally/${event.id}`)}
                    className="bg-plasma-slate/40 backdrop-blur-md rounded-2xl p-6 relative overflow-hidden group border border-white/5 cursor-pointer hover:border-white/20 transition-all hover:bg-plasma-slate/60"
                  >
                    <div className="flex flex-col sm:flex-row gap-6">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {(user?.plasmaUserID === event.organizerID || user?.id === event.organizerID) && (
                            <button
                              onClick={(e) => { e.stopPropagation(); createRallyModal.open(event); }}
                              className="p-2 -mr-2 -mt-2 text-plasma-text-secondary hover:text-plasma-primary transition-colors cursor-pointer group/edit relative z-10"
                              title="Edit Rally"
                            >
                        ...
                        <div className="group/link block">
                          <h4 className="text-xl font-display font-bold mb-1 text-plasma-text-primary group-hover/link:text-plasma-primary transition-colors">{event.title}</h4>
                        </div>
                        ...
                        <div className="space-y-3 mb-6 relative z-10">
                          {event.roles.length > 0 ? (
                            event.roles.map((role, idx) => (
                              <button
                                key={idx}
                                onClick={(e) => { e.stopPropagation(); !event.rsvpd && rsvpModal.open({ ...event, preselectedRoleIdx: idx }); }}
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); !event.rsvpd && rsvpModal.open({ ...event, preselectedRoleIdx: -1 }); }}
                        ...
                        <div className="flex justify-between items-center relative z-10">
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); event.rsvpd ? toggleRSVP(event.id) : rsvpModal.open(event); }}
                            className={`px-6 py-2 rounded-full text-xs font-bold transition-all cursor-pointer ${event.rsvpd
                          ...
                          >
                            {event.rsvpd ? (
                              <>
                                <span className="group-hover/cancel:hidden">✓ RSVP'd</span>
                ...
                <div className="text-center py-12 bg-plasma-slate/20 rounded-2xl border border-white/5">
          ...
          <div className="space-y-12 pb-20 animate-fade-in">
            {loading ? (
                ...
                <article key={sIdx} className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {section.items.map(item => (
                      <div
                        key={item.id}
                        onClick={() => router.push(`/rally/${item.id}`)}
                        className="bg-plasma-slate rounded-xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between group hover:bg-plasma-slate-hover transition-all border border-transparent hover:border-white/20 gap-6 md:gap-0 cursor-pointer"
                      >
                        <div className="flex items-center gap-6 relative w-full md:w-auto">
                          ...
                          <div>
                            <div className="flex items-center justify-between relative z-10">
                              <div className="group/link block">
                                <h3 className="font-display font-bold text-xl text-plasma-text-primary group-hover/link:text-plasma-primary transition-colors">{item.title}</h3>
                              </div>
                              {(user?.plasmaUserID === item.organizerID || user?.id === item.organizerID) && (
                                <button
                                  onClick={(e) => { e.stopPropagation(); createRallyModal.open(item); }}
                        ...
                        <div className="flex items-center gap-10 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-white/5 pt-4 md:pt-0 mt-2 md:mt-0 relative z-10">
                          ...
                          <div className="flex items-center gap-6 md:gap-10">
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); item.rsvpd ? toggleRSVP(item.id) : rsvpModal.open(item); }}
                              className={`px-6 py-2 rounded-full text-xs font-bold transition-all cursor-pointer ${item.rsvpd
                                ? "bg-plasma-success/15 text-plasma-success border border-plasma-success/30 hover:bg-plasma-error/20 hover:text-plasma-error hover:border-plasma-error/30 group/cancel"
                                : "bg-primary-gradient text-white shadow-lg shadow-plasma-secondary/20 hover:scale-105 active:scale-95"
                                }`}
                            >
              ...
              <div className="text-center py-16 bg-plasma-slate/20 rounded-2xl border border-white/5">
