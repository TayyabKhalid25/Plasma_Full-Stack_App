 ...
 * 15 attempts per 10 minutes per IP. Prevents brute-force password attacks
 ...
 * 10 requests per 10 minutes per IP. A user's Steam library does not change
...
const syncLimiter = rateLimit({
    windowMs: 10 * 60 * 1000,   // 10-minute window
    max: 10,                      // 10 sync requests per window per IP
    standardHeaders: true,
    legacyHeaders: false,
    message: {
        success: false,
        errorType: 'SyncRateLimitExceeded',
        message: 'Steam/IGDB sync is limited to prevent API abuse. Please wait 10 minutes before syncing again.',
        retryAfterMs: 10 * 60 * 1000
    }
});
